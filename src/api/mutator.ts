...
export function fieldErrors(error: unknown): Record<string, string> {
  ...
  for (const item of errors as { loc?: unknown[]; msg?: unknown }[]) {
    ...
    if (key && !(key in result))
      result[key] = item.msg.replace(/^Value error, /, "")
  }
  return result
}
...
export function errorMessage(error: unknown): string {
  ...
  if (error.code === "validation_error") {
    const errors = Array.isArray(error.details.errors)
      ? (error.details.errors as { msg?: unknown }[])
      : []
  ...
  if (error.code === "origin_not_allowed")
    return "The server rejected this request's origin. Reload the page and try again."
  if (error.status >= 500)
