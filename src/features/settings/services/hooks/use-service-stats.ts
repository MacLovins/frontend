...
function loadable<T, V>(
  ...
  if (query.data !== undefined)
    return { status: "ready", value: pick(query.data) }
  if (query.isError && missing !== undefined && isNotFound(query.error))
    return { status: "ready", value: missing }
...
export function useServiceStats(
  ...
  return {
    questions: loadable(questions, (list) => {
      ...
      return {
        total: active.length,
        blockers: active.filter((question) => question.polarity === "negative")
          .length,
      }
    }),
    rules: loadable(
      rules,
      (list) => list.filter((rule) => rule.is_active).length
    ),
    icp: loadable<ICPProfileOut, ICPProfileOut | null>(
      icp,
      (profile) => profile,
      null
    ),
    scoringVersion: loadable<ScoringProfileOut, number | null>(
      scoring,
