...
export const toneOf = (polarity: Reason["polarity"]): MarkTone =>
  polarity === "positive" ? "positive" : polarity === "negative" ? "negative" : "neutral"

export function reasonSource(reason: Pick<Reason, "source_name" | "date">) {
  return [reason.source_name, reason.date ? compactAge(reason.date) : null].filter(Boolean).join(" · ")
}
...
export function pickReasons(reasons: Reason[], max = 3) {
  ...
  const notes = reasons.filter((reason) => reason.polarity === "fit" || reason.polarity === "data_gap")
  const picked = [...positives.slice(0, negatives.length ? max - 1 : max), ...negatives.slice(0, 1)]
