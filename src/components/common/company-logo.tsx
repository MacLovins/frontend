...
const LEGAL_SUFFIXES = new Set([
  "a/s", "ag", "sa", "s.a.", "n.v.", "nv", "gmbh", "group", "ltd", "plc", "inc", "se", "s.p.a.", "spa", "oy",
  "ab", "asa", "b.v.", "bv", "llc", "srl", "s.r.l.", "sas", "kg", "co",
])

function companyInitials(name: string) {
  const words = name.split(/\s+/).filter((word) => word && !LEGAL_SUFFIXES.has(word.toLowerCase()))
  ...
  return (
    <span
      ...
      className={cn(
        ...
        size === "md" ? "size-8 rounded-sm text-xs font-bold" : "size-14 rounded-lg text-base font-extrabold",
        className,
      )}
    >
